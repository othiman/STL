...
char Usage3[] = "                -f  shar <archive>\n\
		-p  use this as prefix to each line in archived files\n\
                -u  unshar <archive>\n";

char Usage4[] = "                -v  verbose on extraction, incl. echoing filesizes\n";
...
#endif
  if (pchName)
    fprintf (out, "# By:\t%s (%s)\n", pchName, pchOrg ? pchOrg : "Dave Wecker Midnight Hacks");
  return (0);
}
     ...
     char *line, *sea, *par;
{
  ...
  while (*line) {
    ...
    if (*scr2 == 0) {
      ...
      while (*scr1 == ' ' || *scr1 == '\t' || *scr1 == '\\' || *scr1 == '\'' || *scr1 == '"')
        scr1++;
      while (*scr1 != 0 && *scr1 != ' ' && *scr1 != '\t' && *scr1 != '\\' && *scr1 != '\'' && *scr1 != '"' && *scr1 != '\n' && *scr1 != '\r')
        *par++ = *scr1++;
      *par = 0;
      return;
    }
     ...
     char *filename;
{
  ...
  /* If it already is in "CPM" format we'll check if we need to truncate the front filename part. */
  ...
  /* OK, filename is longer than can be handled, and it doesnt have a filetype "." marker already.  We will put one in to minimize truncation. */
